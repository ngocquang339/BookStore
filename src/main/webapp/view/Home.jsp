...
<c:if test="${sessionScope.user != null and (sessionScope.user.role == 1 or sessionScope.user.role == 3)}">
    ...
    <main class="main-content container" style="margin-top: 20px;">
        ...
        <c:if test="${empty listBooks}">
            ...
            <%-- CHỈ HIỂN THỊ KHI CÓ CHƯƠNG TRÌNH KHUYẾN MÃI ĐANG CHẠY VÀ CÓ SÁCH BÊN TRONG --%>
<c:if test="${not empty activePromo and not empty flashSaleBooks}">
    <section class="flash-sale-section">
        <div class="container">
            <div class="fs-header">
                <div class="fs-header-left">
                    <div class="fs-title">
                        <i class="fa-solid fa-bolt"></i> ${activePromo.promoName}
                    </div>
                    <div class="fs-timer">
                        <span>Kết thúc trong</span>
                        <div class="timer-box">
                            <span id="fs-hours">00</span> : 
                            <span id="fs-minutes">00</span> : 
                            <span id="fs-seconds">00</span>
                        </div>
                    </div>
                </div>
                            ...
                            <c:forEach items="${flashSaleBooks}" var="b" begin="0" end="9">
                                <div class="fs-card-item">
                                    <div class="fs-card-content">
                                        <div class="fs-image">
                                            <a href="${pageContext.request.contextPath}/detail?pid=${b.id}">
                                                <img src="${pageContext.request.contextPath}/${b.imageUrl}" alt="${b.title}" onerror="this.src='https://placehold.co/200x300?text=No+Image'">
                                            </a>
                                            <div class="fs-discount-badge">-${activePromo.discountPercent}%</div>
                                        </div>
    ...
    <c:if test="${not empty activePromo}">
        <script>
            // Lấy thời gian kết thúc từ Server gửi sang (Dạng chuẩn ISO)
            var endDateString = "${activePromo.endDate}".replace(" ", "T"); 
            var countDownDate = new Date(endDateString).getTime();

            var x = setInterval(function() {
                // Bảo hiểm: Kiểm tra xem giao diện đồng hồ có thực sự tồn tại trên màn hình không
                var fsHours = document.getElementById("fs-hours");
                if (!fsHours) {
                    clearInterval(x); // Xóa đồng hồ ngầm đi cho nhẹ máy
                    return;
                }
                ...
                if (distance < 0) {
                    clearInterval(x);
                    // Nếu hết giờ thì hiển thị số 0
                    fsHours.innerHTML = "00";
                ...
                // Thêm số 0 đằng trước nếu nhỏ hơn 10 (vd: 09 thay vì 9)
                fsHours.innerHTML = hours < 10 ? "0" + hours : hours;
                document.getElementById("fs-minutes").innerHTML = minutes < 10 ? "0" + minutes : minutes;
                document.getElementById("fs-seconds").innerHTML = seconds < 10 ? "0" + seconds : seconds;
            }, 1000);
        </script>
    </c:if>
    </body>
