...
<head>
    ...
    <style>
        /* CSS for Admin Badges (In case not in home.css yet) */
        .badge-hidden { background: #555; color: white; font-size: 10px; padding: 2px 5px; border-radius: 3px; position: absolute; top: 5px; right: 5px; z-index: 10; }
        .badge-out-of-stock { background: #dc3545; color: white; font-size: 10px; padding: 2px 5px; border-radius: 3px; position: absolute; top: 25px; right: 5px; z-index: 10; }
        .book-card, .product-card { position: relative; } /* Needed for absolute positioning of badges */
    </style>
</head>
...
<body class="${sessionScope.user != null && sessionScope.user.role == 1 ? 'admin-active' : ''}">
    ...
    <main class="main-content container">
                ...
                <c:forEach items="${listBooks}" var="b">
                    <div class="product-card">
                        <a href="detail?id=${b.id}">
                            <img src="${pageContext.request.contextPath}/assets/image/${b.image}" alt="${b.title}">
                        </a>
        ...
        <h2 style="margin-top: 20px; border-left: 4px solid #C92127; padding-left: 10px;">Sách Nổi Bật</h2>
        <div class="book-list">
            <c:forEach items="${randomBooks}" var="b">
                <div class="book-card">
                    
                    <c:if test="${!b.active && sessionScope.user.role == 1}">
                        <span class="badge-hidden"><i class="fa-solid fa-eye-slash"></i> HIDDEN</span>
                    </c:if>
                    ...
                    <div class="book-info">
                        <h3 class="book-title">
                            <a href="detail?id=${b.id}" style="text-decoration: none; color: inherit;">
                                ${b.title}
                            </a>
                        </h3>
        ...
        <h2 style="margin-top: 40px;">Sách Mới (New Arrivals)</h2>
