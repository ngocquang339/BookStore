...
<head>
    ...
    <style>
        ...
        .card-custom { border-radius: 10px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); border: none; }
        .checkbox-lg { width: 20px; height: 20px; cursor: pointer; }
        .row-picked { background-color: #d1e7dd !important; transition: background-color 0.3s; }
    </style>
</head>
<body>
...
<div class="container mt-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h3 class="fw-bold"><i class="fa-solid fa-boxes-packing text-primary"></i> Danh Sách Lấy Hàng (Picking List)</h3>
    ...
    <form method="post" action="${pageContext.request.contextPath}/warehouse/picking" onsubmit="return validatePickingForm()">
        ...
        <div class="card card-custom">
                ...
                <table class="table table-hover align-middle table-bordered mb-0">
                    <thead class="table-dark">
                        <tr>
                            ...
                            <th>Tên Sách</th>
                            <th class="text-center">Vị trí kho (Location)</th>
                            <th class="text-center">Số Lượng Cần Lấy</th>
                            <th class="text-center">Đã Lấy (Picked)</th>
                        </tr>
                    </thead>
                    <tbody>
                        <c:forEach var="item" items="${pickingItems}">
                            <tr class="item-row">
                                <td class="text-center fw-bold text-secondary">#${item.book_id}</td>
                                <td class="fw-bold">${item.title}</td>
                                <td class="text-center"><span class="badge bg-info text-dark">${item.location}</span></td>
                                <td class="text-center fs-5 fw-bold text-danger">${item.quantity}</td>
                                <td class="text-center">
                                    <input type="checkbox" class="form-check-input checkbox-lg pick-checkbox" 
            ...
            <div class="card-footer bg-white d-flex justify-content-end p-3 border-top">
                <div class="me-4 d-flex align-items-center">
                    ...
                    <span id="progressText" class="badge bg-secondary fs-6">0 / ${pickingItems.size()}</span>
                </div>
                
                <button type="submit" class="btn btn-success btn-lg px-4 shadow-sm" id="btnConfirm">
                    <i class="fa-solid fa-check-double"></i> Confirm & Pack
                </button>
            </div>
        </div>
    </form>
