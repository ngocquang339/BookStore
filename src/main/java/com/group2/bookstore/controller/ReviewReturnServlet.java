...
public class ReviewReturnServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // --- 🛡️ BACKEND SECURITY CHECK START 🛡️ ---
        if (originalReq != null) {
            if (newStatus == 8 && originalReq.getStatus() != 8) {
                // RED ALERT: Block the transaction!
                request.getSession().setAttribute("errorMessage", "Security Violation: Only Warehouse staff can mark an item as 'Ready for Refund' (Passed QC).");
                response.sendRedirect(request.getContextPath() + "/admin/returns");
                return; // Stop code execution immediately!
            }
        }
        // --- 🛡️ BACKEND SECURITY CHECK END 🛡️ ---
